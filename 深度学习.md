...
全称： Shifted Window Transformer

## 基础概念

Swin Transformer是一种基于Transformer的视觉模型，由Microsoft研究团队提出，旨在解决传统Transformer模型在计算机视觉任务中的高计算复杂度问题。

通过引入分层架构和滑动窗口机制，Swin Transformer在性能和效率之间取得了平衡。
...
| 对比     | 传统学习                         | 迁移学习                           |
| -------- | -------------------------------- | ---------------------------------- |
| 数据分布 | $P_{train}(x, y)=P_{test}(x, y)$ | $P_{train}(x, y) ≠ P_{test}(x, y)$ |
| 数据标注 | 有标注                           | 很少或者没有标注                   |
| 模型     | 从头开始构建模型                 | 从源模型迁移                       |
| 计算量   | 巨大算力                         | 有限计算                           |
...
- Domain adaptation域自适应
- Zero-shot / few-shot learning零样本/少样本学习
- Meta learning / Lifelong learning元学习/持续学习
- Multi-task learning多任务学习
- Covariate shift adaptation协变量偏移学习

零样本学习：模型在**完全没有见过该任务的训练样本**的情况下，直接完成任务

Meta learning 是让模型不仅学任务本身，而是**学习一种“快速适应新任务的能力”**

多任务学习：让一个模型在训练时**同时学习多个相关任务**，并通过共享信息来提升整体表现

协变量偏移学习：当输入数据分布变了，但“规律没变”，如何让模型继续有效

👉 **冷启动问题（Cold Start Problem）**

当一个系统**缺乏历史数据**时，无法有效进行预测或推荐，这种情况就叫：

### 怎么迁移

- 经验风险最小化（Empirical risk minimization, ERM）
  - $f^{*}=arg \space min_{f} \frac{1}{m} L \big( f(x_i), y_i \big)$，$L(.,.)$ 是损失函数
- 迁移学习
  - 条件：$P(x,y) \neq Q(x,y)$分布不同，或者$T_s \neq T_t$任务不同
  - 在ERM中正则化这个条件
  - $f^{*}=arg \space min_{f} \frac{1}{m} L \big( f(x_i), y_i \big) + \lambda R(x_i, y_i)$

关于怎么表征$R(...)$

1. 如果目标分布其实约等于源分布的子集，那么我们就不需要$R(...)$
2. 用目标分布和源分布的距离来表征$R(...)$
3. 如果已经学得源分布的某个特征表达$f_s$，学习目标就变成用$f_t$逼近$f_s$，相当于隐式优化$R(...)$

#### 三种基本的迁移学习方法

**基于实例的迁移**Instance-based TL

只保留对目标任务有帮助的源数据，并根据重要性赋予不同的权重
...
但是放到深度学习里面，我就有点看不懂，不如说，什么公式只要一离散化，我就看不懂了。

比如说二分类

这个公式就变成了
$$
-ylog\{y_{head}\}-(1-y)log\{y_{head}\}
$$
以前我总把它当作整体来看，比如预测出y1=p1, y2=p2......

我就会把预测结果看成一个概率序列，然后和gt序列做交叉熵。

但是这样理解似乎是错误的。

他这里计算交叉熵的最小实体是单个样本，每个样本就能单独计算交叉熵。这就和我之前刚开始看《机器学习》的时候一直分不清样本数量和属性数量。

比如对于某个样本yi，预测为1的概率=pi，预测为0的概率=1-pi

如果它gt=1，说明它为1的概率是1，为0的概率是0，所以交叉熵=-1·log(pi)-0·log(1-pi)=-log(pi)

=-ylog(pi)

如果它gt=0，说明它为0的概率是1，为1的概率是1，所以交叉熵=-1·log(1-pi)-0·log(pi)=-log(1-pi)
